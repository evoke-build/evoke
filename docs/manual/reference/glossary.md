...
| **Fits**         | The yes/no question per reflex — does it do what was asked? — whose runner-up floor marks an input asking for two things |
| **Flag**         | A yes/no argument; the body receives `true` or nothing                                                    |
| **Floor**        | A threshold the adapter ships, meaning *the probability this is right*: `route`, `fits`, `read`, `write`  |
...
| **Lock**         | `evoke.lock`: per remote reflex, the ref, tag, commit, `h1` and consented effect; and the adapter          |
...
| **Pick**         | An argument read from a verbatim span of the input: `number`, `duration`, `email`, `url`, `quoted`         |
...
| **Span**         | A verbatim piece of the input, by character offsets                                                       |
