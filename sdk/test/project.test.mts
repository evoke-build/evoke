...
test("handle does the whole loop and returns every expected outcome", async () => {
  ...
  if (ran.outcome === "ran") deepStrictEqual(ran.result, { text: "group-7 lights dim", contained: status("file") }) // the word's value reaches the body
  ...
  if (whole.outcome === "ran") deepStrictEqual(whole.result, { text: "den lights off", contained: status("file") })
})
...
test("a reflex that runs a program runs it with the call's values", async () => {
  ...
  equal(d.outcome, "run")
  if (d.outcome !== "run") return
  deepStrictEqual(await project.run(d), { text: "den off", contained: status("argv") })
})
